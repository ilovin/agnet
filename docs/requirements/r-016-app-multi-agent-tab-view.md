...
phone-talk 已落地 Manager-Agent 架构（[[feedback_manager_workflow]]、[[project_manager_architecture]]）：Manager 把所有实际 dev/test 工作拆成子 agent，并发派到独立 worktree 执行。R-008 / R-012 阶段已经多次出现并行 5 个左右 dev agent 的真实场景。
...
**[[i-018-tmux-contentmatch-session-bleed]] — tmux contentmatch 跨 session 串扰**必须先修复。
...
i-018 issue 已立项跟踪。R-016 进入 Phase 4 拆解前，必须先确认 i-018 已 closed 并通过多 agent 并发场景验证。
...
2. **tab 内信息**（每个 tab 对应一个子 agent）
   ...
   - 完成态：agent 完成后 tab 不自动消失，保留以便回看

3. **切换交互**
   - tab 顺序：按 agent 创建时间升序固定，避免抖动
   - 未读徽标：非当前 tab 收到新 conversation event 时显示红点 / 数字徽标
   - 关闭：用户可手动关闭已 completed/error 的 tab；working 状态的 tab 不允许关闭（防止误关）
...
1. **tmux content-match 错绑（i-018）** — 多并发 agent 场景下 jsonl 与 tmux pane 路由不可靠；详见 [[i-018-tmux-contentmatch-session-bleed]] §Root Cause
...
- [ ] tab 数量 = 当前节点活跃 + 最近完成 agent 数（与 dashboard 节点卡显示数一致）
...
- [ ] 非当前 tab 收到新 event 时显示未读徽标
- [ ] completed/error 的 tab 可关闭，working 的 tab 不可关闭
- [ ] tab 切换后滚动位置保留
...
### 与底层修复联动
- [ ] [[i-018-tmux-contentmatch-session-bleed]] 已 closed
...
### 验证
- [ ] 在现有 Chrome 标签页中跑 5 agent 并发场景验证（[[feedback_existing_chrome_only]]）
...
1. **tab 命名规则**：用 task ID（如 `R-008-T6`）还是用 agent name（如 `T6 deploy.sh DEPLOY_DRY_RUN`）作为 tab 标题？task ID 短但需要 task 元数据通道；agent name 长但已经是 Agent 工具 result 现成字段
2. **completed agent 保留时长**：tab 关闭后 agent 数据是否还在 dashboard 计数中？建议关闭后从 tab 列表移除但 dashboard 仍显示历史汇总；待用户确认
3. **tab 数量上限**：5 个常见，10 个+ 是否需要折叠/分页？暂不设上限，用户负担过重再优化
4. **未读徽标重置时机**：切到 tab 立即清零，还是滚动到底部才清零？建议立即清零，与微信/Slack 心智一致
5. **跨设备同步 tab 选择**：用户在 Web 端选了 tab A，移动端是否同步？建议**不同步**，每个客户端独立选择，避免协议开销
6. **多 tab 同时弹交互卡片**：除了徽标，是否需要显眼的全局 toast / 顶部横幅？建议仅徽标，避免打扰；待用户拍板
...
- **i-018 修复时序**：i-018 是 agentd 端 tmux watcher 改动，回归风险大；本需求开工前必须确认 i-018 不仅 close，还要在多 agent 实战场景下稳定运行至少 1 周
...
- 直接前置：[[i-018-tmux-contentmatch-session-bleed]]（必须先修）
- 完整 PRD：[[multi-agent-tab-view-prd]]（i-018 + R-016 合并的分期实施 PRD）
- 相关需求：[[r-009-claude-agent-view-adaptation]]（终端 `claude agents` 工作流）、[[r-010-app-claude-interaction]]（交互卡片渲染与回传）
- 相关设计：[[d-009-agent-view-workflow]]（Manager-Agent View 集成设计）
- 相关 issue：[[i-016-periodic-scan-dead-process-cleanup]]（死亡进程清理 — 影响 tab 自动关闭/切换）、[[i-017-dashboard-stale-last-message-time]]（merge 逻辑 — tab 顺序稳定性可能受影响）
- Manager 工作流：[[manager-workflow]]（必须先 Explore → Decompose → Dev/Test）
