...
func opencodeInsertMessage(t *testing.T, dbPath, msgID, sessionID, role string, ts int64) {
	t.Helper()
	db, err := sql.Open("sqlite", dbPath+"?_pragma=busy_timeout(3000)")
	if err != nil {
		t.Fatalf("open: %v", err)
...
func opencodeInsertPart(t *testing.T, dbPath, partID, msgID, sessionID, partData string, ts int64) {
	t.Helper()
	db, err := sql.Open("sqlite", dbPath+"?_pragma=busy_timeout(3000)")
	if err != nil {
		t.Fatalf("open: %v", err)
...
func TestOpenCodeDBWatcher_QuestionToolEmitsToolUseFields(t *testing.T) {
	...
	opencodeInsertMessage(t, dbPath, "msg_seed", sessionID, "user", 1000)
	opencodeInsertPart(t, dbPath, "part_seed", "msg_seed", sessionID,
		`{"type":"text","text":"hi"}`, 1000)

	// Assistant message with a question tool part.
	opencodeInsertMessage(t, dbPath, msgID, sessionID, "assistant", 2000)
	...
	opencodeInsertPart(t, dbPath, "part_q1", msgID, sessionID, questionPart, 2001)
	...
	opencodeInsertMessage(t, dbPath, "msg_after", sessionID, "user", 3000)
	opencodeInsertPart(t, dbPath, "part_after", "msg_after", sessionID,
...
func TestOpenCodeDBWatcher_RegularToolDoesNotPopulateInteractiveFields(t *testing.T) {
	...
	opencodeInsertMessage(t, dbPath, "msg_bash", sessionID, "assistant", 2000)
	bashPart := `{"type":"tool","tool":"bash","callID":"tool_bash1","state":{"status":"completed","input":{"command":"ls -la"},"output":"file1\nfile2"}}`
	opencodeInsertPart(t, dbPath, "part_bash", "msg_bash", sessionID, bashPart, 2001)
...
func TestOpenCodeDBWatcher_MalformedQuestionInputDoesNotCrash(t *testing.T) {
	...
	opencodeInsertMessage(t, dbPath, "msg_bad", sessionID, "assistant", 2000)
	opencodeInsertPart(t, dbPath, "part_bad", "msg_bad", sessionID,
...
func TestOpenCodeDBWatcher_CompletedQuestionAlsoPopulates(t *testing.T) {
	...
	opencodeInsertMessage(t, dbPath, "msg_q", sessionID, "assistant", 2000)
	...
	opencodeInsertPart(t, dbPath, "part_q", "msg_q", sessionID, completedQ, 2001)
