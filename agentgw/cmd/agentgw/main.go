...
func runServer(tunnelURLFlag, tunnelTokenFlag, hubBaseFlag, appURLFlag string, realityCfg *tunnel.RealityConfig, showQR bool) {
	...
	http.HandleFunc("/v1/release/latest", func(w http.ResponseWriter, r *http.Request) {
		if !checkToken(r, cfg.Token) {
			http.Error(w, "unauthorized", http.StatusUnauthorized)
			return
		}
		manifestURL := "https://download.ilovin.xyz/v1/release/latest"
		if DefaultDownloadDomain != "" {
			manifestURL = "https://" + DefaultDownloadDomain + "/v1/release/latest"
		}
		client := &http.Client{Timeout: 10 * time.Second}
		resp, err := client.Get(manifestURL)
		if err != nil {
			http.Error(w, `{"error":"fetch manifest failed"}`, http.StatusBadGateway)
			return
		}
		defer resp.Body.Close()
		w.Header().Set("Content-Type", "application/json")
