...
build_agentd_linux_amd64() {
    ...
    (cd "$AGENTD_DIR" && GOOS=linux GOARCH=amd64 go build -o "../$LINUX_AMD64_BIN" ./cmd/agentd/)
...
build_agentgw_linux_amd64() {
    ...
    if [[ -n "$ldflags" ]]; then
        ...
        (cd "$AGENTGW_DIR" && GOOS=linux GOARCH=amd64 go build -o "../$GW_LINUX_AMD64_BIN" ./cmd/agentgw/)
    fi
